...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 A status bar item is present after plugin load.
- [x] #2 Status bar displays "Multiplayer: not signed in" when `isAuthenticated` is `false`.
- [x] #3 Status bar displays "● Multiplayer" when signed in and all providers report `connected = true` and `synced = true`.
- [x] #4 Status bar displays "⟳ Multiplayer" when any provider has `synced = false` (initial sync in progress).
- [x] #5 Status bar displays "○ Multiplayer" when any provider has `connected = false` (reconnecting) but there is no auth error.
- [x] #6 Status bar displays "⚠ Multiplayer: sign in again" after a 4001 close or refresh failure.
- [x] #7 Clicking the status bar item in the "sign in again" state opens the settings tab.
- [x] #8 Status bar item is removed on plugin unload.
<!-- AC:END -->
...
Connection status bar item fully implemented in main.ts with reactive updates via auth-changed and y-websocket status/sync events. Includes click-to-open-settings on auth error, priority-based status aggregation, and proper listener attach/detach lifecycle.
<!-- SECTION:FINAL_SUMMARY:END -->

## Definition of Done
<!-- DOD:BEGIN -->
- [x] #1 All acceptance criteria pass manual testing
- [x] #2 No lint errors introduced (`npm run lint`)
- [x] #3 Code follows existing patterns in the codebase
- [x] #4 Status bar updates reactively — no polling
<!-- DOD:END -->
