...
export class AuthManager implements IAuthManager {
	...
	constructor(
		...
		deps: AuthManagerDeps = defaultDeps,
	) {
		this._app = app;
	...
	private async _doSignIn(): Promise<void> {
		...
		const codeChallenge = await this._computeCodeChallenge(
			this._codeVerifier,
		);

		// Step 2: Start loopback HTTP server and wait for callback (RFC 8252 §7.3)
		const { callbackParams, redirectUri } =
	...
	private _startCallbackServer(): Promise<{
		...
		return new Promise((resolveSetup) => {
			...
			const callbackParams = new Promise<AuthCallbackParams>(
				(resolve, reject) => {
			...
			const server = createServer(
				(req: IncomingMessage, res: ServerResponse) => {
					...
					res.end(
						"<html><body><p>Sign-in complete. You can close this tab.</p></body></html>",
					);
