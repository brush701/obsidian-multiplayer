...
export class SharedFolderModal extends Modal {
	...
	onOpen() {
		...
		const sharedFolder = this.plugin.sharedFolders.find((sf) =>
			this.folder.path.contains(sf.settings.path),
		);
		if (sharedFolder) {
			contentEl.createEl("h2", { text: "SharedFolder already exists" });
			contentEl.createEl("p", {
				text: "This folder is already a multiplayer sharedFolder.",
			});
		...
		const createTab = tabBar.createEl("button", {
			text: "Create",
			cls: "multiplayer-tab multiplayer-tab-active",
		});
		const joinTab = tabBar.createEl("button", {
			text: "Join",
			cls: "multiplayer-tab",
		});

		// Tab content containers
		this.createContentEl = contentEl.createDiv({
			cls: "multiplayer-tab-content",
		});
		this.joinContentEl = contentEl.createDiv({
			cls: "multiplayer-tab-content",
		});
		this.joinContentEl.style.display = "none";

		createTab.onClickEvent(() =>
			this.switchTab("create", createTab, joinTab),
		);
...
export class UnshareFolderModal extends Modal {
	...
	onOpen() {
		...
		modalEl.addClass("modal-style-multiplayer");
		contentEl.empty();
		const sharedFolder = this.plugin.sharedFolders.find(
			(sharedFolder) =>
				this.folder.settings.path == sharedFolder.settings.path,
		);
		if (sharedFolder) {
			...
			button.onClickEvent((ev) => {
				this.plugin.settings.sharedFolders =
					this.plugin.settings.sharedFolders.filter(
						(el) => el.path !== sharedFolder.settings.path,
					);
				this.plugin.sharedFolders = this.plugin.sharedFolders.filter(
					(el) => el.settings.path !== sharedFolder.settings.path,
				);
...
export class InviteModal extends Modal {
	...
	onOpen() {
		...
		contentEl.createEl("h2", {
			text: `Invite to "${this.sharedFolder.settings.name}"`,
		});
		...
		const roleContainer = contentEl.createDiv({
			cls: "multiplayer-role-container",
		});
		roleContainer.style.display = "flex";
		roleContainer.style.gap = "16px";

		const editorLabel = roleContainer.createEl("label");
		const editorRadio = editorLabel.createEl("input", {
			type: "radio",
			attr: { name: "invite-role", value: "EDITOR" },
		}) as HTMLInputElement;
		editorRadio.checked = true;
		editorLabel.appendText(" Editor");

		const viewerLabel = roleContainer.createEl("label");
		const viewerRadio = viewerLabel.createEl("input", {
			type: "radio",
			attr: { name: "invite-role", value: "VIEWER" },
		}) as HTMLInputElement;
		viewerLabel.appendText(" Viewer");

		editorRadio.addEventListener("change", () => {
			if (editorRadio.checked) this.selectedRole = "EDITOR";
		});
		viewerRadio.addEventListener("change", () => {
			if (viewerRadio.checked) this.selectedRole = "VIEWER";
		});
		...
		const expirySelect = contentEl.createEl("select", {
			cls: "multiplayer-expiry-select",
		...
		for (const opt of options) {
			const optionEl = expirySelect.createEl("option", {
				text: opt.label,
		...
		expirySelect.addEventListener("change", () => {
			this.selectedExpiry = expirySelect.value as InviteExpiry;
		});
	...
	async loadMembers() {
		try {
			const [room, myRole] = await Promise.all([
				...
				this.plugin.apiClient.getMyRole(
					this.sharedFolder.settings.guid,
				),
			]);
			...
			const listEl = this.bodyEl.createEl("div", {
				cls: "multiplayer-members-list",
			});
			...
			for (const member of sorted) {
				...
				row.createSpan({
					text: member.email,
					cls: "multiplayer-member-email",
				});
				row.createSpan({
					text: formatRole(member.role),
					cls: "multiplayer-member-role",
				});
			}

			// Action buttons
			const actions = this.bodyEl.createDiv({
				cls: "multiplayer-members-actions",
			});
