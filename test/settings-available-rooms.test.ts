...
let lastFolderSelectCallback: ((folder: { path: string }) => Promise<void>) | null = null
vi.mock('../src/modals', () => ({
  FolderSelectModal: class {
    constructor(_app: unknown, cb: (folder: { path: string }) => Promise<void>) {
      lastFolderSelectCallback = cb
    }
    open() {}
  },
  SharedFolderModal: class { open() {} },
  UnshareFolderModal: class { open() {} },
  InviteModal: class { open() {} },
  MembersModal: class { open() {} },
}))
...
describe('Available rooms in settings tab', () => {
  ...
  describe('unauthenticated', () => {
    it('shows sign-in message and does not call listRooms', () => {
      ...
      const { tab, containerEl } = buildTab({
        apiClient: { listRooms },
        authManager: {
          isAuthenticated: false, userInfo: null, hasAuthError: false,
          on: vi.fn(), off: vi.fn(),
        },
      })
  ...
  describe('error', () => {
    it('shows error message on API failure', async () => {
      ...
      expect(allTexts(getAvailableRoomsEl(containerEl))).toContain('Could not load rooms.')
    })
  })
  ...
  describe('empty', () => {
    it('shows empty message when no rooms available', async () => {
      ...
      await waitForLoad(containerEl)
      expect(allTexts(getAvailableRoomsEl(containerEl))).toContain('No additional rooms available.')
    })
  })
  ...
  describe('room list', () => {
    it('renders available rooms (not empty/error)', async () => {
      const rooms = [
        makeRoomListItem({ guid: 'r1', name: 'Q4 Planning', role: 'EDITOR' }),
        makeRoomListItem({ guid: 'r2', name: 'Board Docs', role: 'VIEWER' }),
      ]
      const { tab, containerEl } = buildTab({
    ...
    it('excludes rooms already in settings.sharedFolders', async () => {
      ...
      const { tab, containerEl } = buildTab({
        apiClient: { listRooms: vi.fn().mockResolvedValue(rooms) },
        settings: {
          serverUrl: 'https://example.com', username: '',
      ...
      await waitForLoad(containerEl)
      expect(allTexts(getAvailableRoomsEl(containerEl))).not.toContain('No additional rooms available.')
    })
  ...
  describe('add to vault', () => {
    it('saves settings, creates SharedFolder, and calls addSharedFolder', async () => {
      const rooms = [makeRoomListItem({ guid: 'room-1', name: 'New Room' })]
      ...
      expect(plugin.settings.sharedFolders).toContainEqual({
        guid: 'room-1', name: 'New Room', path: 'my-folder',
      })
    ...
    it('blocks add when folder overlaps an existing shared folder', async () => {
      ...
      const { tab, plugin, containerEl } = buildTab({
        apiClient: { listRooms: vi.fn().mockResolvedValue(rooms) },
        sharedFolders: [{ settings: { guid: 'existing', path: 'shared' } }],
      })
  ...
  describe('race condition', () => {
    it('discards stale load when re-rendered before API resolves', async () => {
      ...
      const firstLoad = new Promise<unknown[]>(r => { resolveFirst = r })
    ...
    it('registers on display() and unregisters on hide()', () => {
      ...
      const { tab } = buildTab({
        authManager: {
          isAuthenticated: true,
          userInfo: { email: 'test@example.com', name: 'Test' },
          hasAuthError: false, on: onFn, off: offFn, signOut: vi.fn(),
        },
      })
